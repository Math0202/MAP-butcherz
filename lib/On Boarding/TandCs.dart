...
class _TandCsState extends State<TandCs> {
  ...
  @override
  void dispose() {
    _scrollController.removeListener(_onScroll); // Best practice to remove listener
  ...
  void _onScroll() {
    ...
    if (_scrollController.position.extentAfter < 100) { // Enable when near the bottom
      if (!_isButtonEnabled) { // Only call setState if the state changes
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea( // Ensures content avoids notches/system UI
        child: Padding(
          padding: const EdgeInsets.fromLTRB(16.0, 0, 16.0, 16.0), // Add horizontal and bottom padding
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: Container(
                  ...
                  margin: const EdgeInsets.only(top: 16.0), // Add space above the box
                  ...
                  child: Scrollbar( // Add a scrollbar for visual feedback
                    thumbVisibility: true, // Always show scrollbar thumb
                    controller: _scrollController,
                    child: SingleChildScrollView(
                      controller: _scrollController,
                      padding: const EdgeInsets.symmetric(horizontal: 8.0), // Padding inside scroll view
                      child: Text(
                        ...
                        style: theme.textTheme.bodyMedium?.copyWith( // Use theme's body style
              ...
              FilledButton( // Using Material 3 FilledButton
                style: FilledButton.styleFrom(
                  backgroundColor: _isButtonEnabled
                      ? theme.colorScheme.primary // Use primary color when enabled
                      : Colors.grey[400], // Grey out when disabled
                  padding: const EdgeInsets.symmetric(vertical: 16.0), // Taller button
                  shape: RoundedRectangleBorder( // Rounded corners
                    borderRadius: BorderRadius.circular(12),
                  ),
                  textStyle: theme.textTheme.titleMedium?.copyWith( // Use theme's title style
                    fontWeight: FontWeight.bold,
                    color: Colors.white, // Ensure text is white on colored button
                  ),
                ).copyWith(
                ...
                child: const Text('ACCEPT & CONTINUE'), // Clearer button text
              ),
            ],
          ),
        ),
  ...
  void navigateToNextScreen(BuildContext context) {
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(
        builder: (context) => const LoginScreen(), // Navigate to TeamQueryScreen
      ),
    );
  }
}