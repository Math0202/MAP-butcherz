...
class Player {
  final String id;
  final String name; // This will be mapped from fullName
  final String email;
  final String phone;
  ...
  factory Player.fromMap(Map<String, dynamic> map) {
    ...
    return Player(
      ...
      name:
          map['fullName'] ??
          map['name'] ??
          '', // Try fullName first, then name as fallback
      email: map['email'] ?? '',
      phone: map['phone'] ?? '',
...
class _ManageRosterScreenState extends State<ManageRosterScreen> {
  ...
  Future<void> _fetchPlayers() async {
    ...
    try {
      ...
      // Debug: Check what fields exist in the documents
      ...
      print('Found ${playersDocs.length} player documents');

      // Debug: Print the first few documents to see the structure
      for (int i = 0; i < playersDocs.length && i < 3; i++) {
        print('Player doc $i: ${playersDocs[i]}');
      }
      ...
      final players =
          playersDocs.map((doc) {
            ...
            print(
              'Processing player: fullName=${docMap['fullName']}, email=${docMap['email']}',
            );
  ...
  Future<void> _assignUserToTeam(
    ...
    try {
      ...
      if (result.isSuccess && result.nModified > 0) {
        ...
        _fetchPlayers(); // Refresh the roster to show the newly added player
  ...
  Future<void> _updatePlayerTeamDetails(
    ...
    try {
      ...
      if (result.isSuccess && result.nModified > 0) {
        ...
        _fetchPlayers(); // Refresh the roster
      } else if (result.isSuccess && result.nModified == 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              ...
              ? Center(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        _fetchPlayers(); // Refresh the data
                      },
